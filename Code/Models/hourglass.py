...
class Model(nn.Module):
	def __init__(self,channel_scale_factor=1):
		...
		self.seq = nn.Sequential(
			...
			nn.ReLU(True),# output 2,64,512,512
			Channels4(channel_scale_factor),#output 2,32,512,512
			nn.Conv2d(64//channel_scale_factor, 1, 3, padding=1)#output 2,1,512,512
		)

		self.sigmoid=nn.Sigmoid()

	def forward(self,x):
		return self.seq(torch.cat([x],dim=1))#+y torch.cat([x],dim=1) 2,3,512,512
