...
# @timeCost
# @numba.jit
def bspline_interp(P,k,NodeVector,nums):
    path=[]
    Bik_u = np.zeros((len(P), 1))
    for u in np.arange(0, 1, 1/nums):
...
if __name__=='__main__':
    ...
    if flag == 1:  # 均匀B样条很简单
        ...
        NodeVector = U_piecewise_B_Spline(len(P)-1, k)
        path = bspline_interp(P,k,NodeVector=NodeVector)
    path = np.array(path)
