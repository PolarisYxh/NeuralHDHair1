...
class VGGLoss(nn.Module):
    ...
    def __init__(self, model='vgg16', gpu_ids=[],layer=8, shift=0, reduction='mean'):
        super().__init__()
        self.shift = shift
        self.reduction = reduction
        ...
        device = torch.device("cuda") if torch.cuda.is_available() and len(gpu_ids)>0 else torch.device("cpu")
    ...
    def forward(self, input, target, target_is_features=False):
        ...
        return F.mse_loss(input_feats, target_feats, reduction=self.reduction)#二范数的平方
...
def main():
    ...
    crit_vgg = VGGLoss(model='vgg19', layer=35).to(device)
    # crit_l2 = nn.MSELoss()
    # crit_tv = TVLoss(p=1)
    target = tio.read_image('/home/yxh/Documents/company/NeuralHDHair/Code/Loss/DSC00261.jpg')[None] / 255
    target=cv2.imread(f"/home/yxh/Documents/company/NeuralHDHair/data/test/strand_map/0aceffcddc3159d08830e738c63812fc.png")#R:（0,1）表示（向右，向左）；G：第二通道，（0,1）表示（向下，向上）
    # TODO:两种方式得到的segment图不太一样，哪个比较好 后续进行实验
    target0=cv2.resize(target, (224, 224))
    # cv2.imshow("1",gt_parsing)
    # cv2.waitKey()
    target0=target0.transpose([2,0,1])
    target0=torch.from_numpy(target0)[None] / 255
    M_sum = torch.sum(target0[:,2,:,:])
    target1=cv2.resize(target, (224, 224))
    target1=target1.transpose([2,0,1])
    target1=torch.from_numpy(target1)[None] / 255
    # target1 = TF.resize(target, (224, 224), 3)
    target1 += (torch.rand_like(target0) - 0.5) / 4
    target1 = target1.clamp(0, 1)
    TF.to_pil_image(target1[0]).save('target.png')
    # target = tio.read_image('/home/yxh/Documents/company/NeuralHDHair/Code/Loss/DSC00261.jpg')[None] / 255
    # target0 = TF.resize(target, (224, 224), 3)
    # target1 = TF.resize(target, (224, 224), 3)
    # target1 += (torch.rand_like(target0) - 0.5) / 4
    # target1 = target1.clamp(0, 1)
    ...
    try:
        for i in range(250):
            ...
            loss += loss_fn(input, target0)/(3*M_sum)
            # loss += crit_l2(input, target1) * 1500
            print(i, loss.item())
            loss.backward()
            opt.step()
