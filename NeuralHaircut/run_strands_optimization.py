...
class Runner:
    def __init__(self, conf_path, case='CASE_NAME', scene_type='DATASET_TYPE', hair_conf_path=None, exp_name=None):
        ...
        if exp_name is not None:
            ...
            exps_dir = Path('./NeuralHaircut/exps_second_stage') / exp_name / case / Path(conf_path).stem
            cur_dir = date + '_' + time   
...
#         Upload strand-based geometry         
    ...
    def train(self,image,mask,orientation_2d,ori,occ,cam_extr,cam_intr,cam_pos):
        ...
        for iter_i in tqdm(range(res_step)):
            ...
            orig_data_size = image.shape[1] # H shape, suppose square images
            
            raster_dict = {}
            raster_dict['iter'] = self.iter_step

            
            raster_dict['cam_extr'] = cam_extr.to(self.device).to(torch.float32)#torch [4,4]
            raster_dict['cam_pos'] = cam_pos.to(self.device).to(torch.float32)
            if orig_data_size is None:
                raster_dict['cam_intr'] = cam_intr.to(self.device).to(torch.float32)#torch [4,4]
                raster_dict['gt_silh'] = image.permute(2, 0, 1).cuda()
                raster_dict['gt_rgb'] = image.permute(2, 0, 1).cuda()
            else:
                # need to change cameras intrinsic as we render in resolution 512x512
                scale_factor = orig_data_size / self.img_size
                raster_dict['cam_intr'] = scale_mat(deepcopy(cam_intr), scale_factor).to(self.device).to(torch.float32)
                raster_dict['gt_silh'] = F.interpolate(mask.permute(2, 0, 1)[None], size=self.img_size, mode='bilinear')[0].cuda()
                raster_dict['gt_rgb'] = F.interpolate(image.permute(2, 0, 1)[None], size=self.img_size, mode='bilinear')[0].cuda()

            raster_dict['visual_gt_orients'] = orientation_2d
            
            losses.update({
                'hair_' + str(key): val for key, val in self.hair_primitives_trainer.train_step(ori,occ, it=self.iter_step, raster_dict=raster_dict).items()
                })
