...
class SdfChamfer:
    def __init__(self, device='cuda', mesh_outer_hair_remeshed=None,  mesh_outer_hair=None, num_points=10000):
        ...
        self.mesh_outer_hair = IO().load_mesh(mesh_outer_hair, device=device)#hair_outer.ply extract_visible_surface.py
        # to ease the calculation of points2face distanse
        self.mesh_outer_hair_remeshed =IO().load_mesh(mesh_outer_hair_remeshed, device=device) # remesh hair_outer.ply in meshlab
    
def points2face(mesh, points):#remesh3d
...
def calc_chamfer(mesh_outer_hair, points, num_points=10000):#
    # sample points from visible outer hair surface
    sample_points  = sample_points_from_meshes(mesh_outer_hair, num_points)
