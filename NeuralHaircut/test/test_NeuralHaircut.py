...
def pyrender_extri2pytorch3d_extri(cam_extri):
    # pytorch3d y轴相反
    cam_extri[0,3]=-cam_extri[0,3]
    cam_extri[2,3]=-cam_extri[2,3]
    rot_matrix = transform.SimilarityTransform(rotation=[0,np.deg2rad(180),0],dimensionality=3).params
    R = np.linalg.inv(cam_extri[:3,:3])@rot_matrix[:3,:3]
    T = cam_extri[:3,3]
    cam_extri[:3,:3] = R
    cam_extri[:3,3] = T
    return cam_extri
if __name__=="__main__":
    ...
    # cam_extri = pyrender_extri2pytorch3d_extri(cam_extri)
    
    cam_extri = torch.from_numpy(cam_extri).to(torch.float32)
    cam_intri = torch.from_numpy(cam_intri).to(torch.float32)
    ori2D = cv2.resize(ori2D, (640, 640), interpolation=cv2.INTER_CUBIC)
    # orientation = torch.from_numpy(ori)
    # ori : H,W,D
    cam_pos =torch.tensor([[ 9.61313906e-01,  7.01898160e-02, -2.66362467e-01, -7.95505687e-02],
                    [ 6.93889390e-18,  9.66990113e-01,  2.54813897e-01, 1.65589804e+00],
                    [ 2.75455212e-01, -2.44956142e-01,  9.29581042e-01, 2.64301285e-01],
                    [ 0.00000000e+00,  0.00000000e+00,  0.00000000e+00, 1.00000000e+00]]).to(torch.float32)
    
    runner.train(image,mask,ori2D,ori,out_occ,cam_extri,cam_intri,cam_pos)#out_occ:[256,256,192,1] DHW3